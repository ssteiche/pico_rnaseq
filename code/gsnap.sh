...
$MAPPER -t 12 -d $SPECIES --gunzip $KNOWN_SPLICE $MAX_MM --speed 2 --use-shared-memory 0 -n 1 --novelsplicing 1 -i 2 --nofails --read-group-id=$SAMPLE --read-group-name=$SAMPLE --read-group-library=$SAMPLE --read-group-platform=illumina -A sam $R1 $R2 > alignments/$SAMPLE.sam
