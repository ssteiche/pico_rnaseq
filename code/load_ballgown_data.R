...
load_ballgown_data <- function(ballgown.path = "ballgown", pheno.path = "data/stress_expt_meta.txt", fpkm.threshold = 5)
{
  pheno_data   <- read.delim(pheno.path)
  pheno_data   <- arrange(pheno_data, sample)
  express_data <- ballgown(dir(ballgown.path, full.names = TRUE), pData = pheno_data)
